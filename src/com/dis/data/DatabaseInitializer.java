...
public class DatabaseInitializer {
	public static void init() {
		...
		try {
			...
			// statement.execute("CREATE SCHEMA DIS");
			statement.execute("SET SCHEMA DIS");

			// drop all tables
			System.out.print("Dropping tables... ");
			dropTables(statement, new String[] { "TRANSACTIONS", "ARTICLES", "COUNTRIES", "PRODUCTCATEGORIES",
					"PRODUCTFAMILIES", "PRODUCTGROUPS", "REGIONS", "SHOPS", "CITIES" });
			...
			statement.execute(
					"CREATE TABLE ARTICLES (ID INT PRIMARY KEY NOT NULL, NAME VARCHAR(255) NOT NULL, PRICE DOUBLE NOT NULL )");
			statement.execute("CREATE TABLE COUNTRIES (ID INT PRIMARY KEY NOT NULL, NAME VARCHAR(255) NOT NULL)");
			statement.execute(
					"CREATE TABLE PRODUCTCATEGORIES (ID INT PRIMARY KEY NOT NULL, NAME VARCHAR(255) NOT NULL)");
